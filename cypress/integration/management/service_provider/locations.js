...
describe('Location Management', () => {
    ...
    const serviceProviderEmail = 'testuser+service_provider@eservsol.com';
    const password = 'testuser123';

    beforeEach(() => {
        cy.visit('');
        cy.get('input[formcontrolname="email"]').click().type(serviceProviderEmail);
        cy.get('input[formcontrolname="password"]').type(password);
        cy.get('app-login').get('ion-button[type=submit]').last().click();
    })

    it('user can access location management page as service provider', () => {
        cy.get('app-home').get('ion-menu-button').last().click();
        cy.contains('Location Management').click();
        cy.get('ion-menu').click({force: true});
    });

    it('user can add new location as service provider', () => {
    ...
    it('user can add edit location as service provider', () => {
        const location = new Date().getTime() + ' Standort';
        cy.get('app-home').get('ion-menu-button').last().click();
        cy.contains('Location Management').click();
